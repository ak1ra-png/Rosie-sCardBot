...
client.on("friendMessage", (SENDER, MSG) => {
    ...
    if (MSG.toUpperCase().indexOf("!LEVEL") >= 0) {
		...
		client.chatMessage(SENDER, " \n List of commands: \n • !level [your dream level] - calculate how many sets you need and how many keys it'll cost to reach your desired level \n • !check [amount of keys] - shows the current rate. \n \n • !buy [amount of keys] - use to spend that amount of CS:GO keys for sets \n • !keylist - use to show the current list of accepted keys \n \n • !owner - the link to the owners steam profile <3 \n • Our Discord server: https://discord.gg/yqd8rFX");
	}
		...
		client.inviteToGroup(SENDER.getSteamID64(),CONFIG.ID);
		console.log("[ UserChat ] INVITED:", SENDER.getSteamID64(), "TO:", CONFIG.ID);
...
client.on("friendRelationship", (SENDER, REL) => {
    if (REL === 2) {
        ...
        if (CONFIG.ID) {
            client.inviteToGroup(SENDER, CONFIG.ID);
        }
...
manager.on("sentOfferChanged", (OFFER, OLDSTATE) => {
    if(OFFER.state == 2) {
        ...
        if (CONFIG.ID) {
            client.inviteToGroup(OFFER.partner, CONFIG.ID);
        }
